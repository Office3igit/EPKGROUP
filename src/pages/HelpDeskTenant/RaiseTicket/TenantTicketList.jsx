...
function Tenant_ticket_list() {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch('https://epkgroup.in/crm/api/public/api/tenant_view_newraiseticket_list', {
    ...
    //         data: csvData,
    //         headers: headers,
    //         filename: 'Total_Ticket_List.csv',
    //     };
    ...
    const handleExportCSV = () => {
        const csvData = filteredleaveData.map((row, index) => ({
            ...
            TenantID: !tenant_identification ? row.tnt_id : '',
            TenantName: !tenant_identification ? row.tnt_name : '',
            Status: row.status_description,
            Attachment: row.attachment !== '-' ? `https://epkgroup.in/crm/api/storage/app/${row.attachment}` : 'No Attachment',
        }));
        ...
        const headers = [
            ...
            !tenant_identification ? { label: 'Tenant ID', key: 'TenantID' } : null,
            !tenant_identification ? { label: 'Tenant Name', key: 'TenantName' } : null,
    ...
    const handleExportPDF = () => {
        ...
        const headers = [['S.No', 'Task ID', 'Raised Date', 'Department', 'Issues Type', 'Tenant ID', 'Tenant Name', 'Status', 'Attachment']];
        ...
        const data = filteredleaveData.map((row, index) => [
            ...
            row.department_name,
            row.issue_name,
            !tenant_identification ? row.tnt_id : '',  // Conditional Tenant ID
            !tenant_identification ? row.tnt_name : '', // Conditional Tenant Name
    ...
    const Escalaltion_Mail = async (id) => {
        ...
        try {
            const response = await fetch(`https://epkgroup.in/crm/api/public/api/for_tenant_escalation_mail/${id}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${usertoken}`
        ...
        if (row.escalation_status === 1) {
            return (
                <button
                    ...
                    onClick={() => alert("Already escalated mail sent to admin")}
                >
            ...
            if (hoursDifference >= 24) {
                return (
                    <button
                        ...
                        onClick={() => {
                            // Logic to send escalation mail
                            const isConfirmed = window.confirm(`Are you sure you want to send the escalation mail for ticket ID ${row.ticket_id}?`);
                            if (isConfirmed) {
                                // Logic to send escalation mail
                                // alert(`Escalation mail sent for ticket ID ${row.id}`);
                                Escalaltion_Mail(row.id);
                            }
                        }}
                    >
                        <FontAwesomeIcon icon={faBell} />
                ...
                return (
                    <button
                        ...
                        onClick={() => alert("Not yet reached 24 hours for this ticket escalation")}
                    // Disable button as it's not ready for escalation
                    >
                        <FontAwesomeIcon icon={faBell} />
                    </button>
                );
        ...
        <>
            ...
            {loading ? (
                ...
                <Container fluid className='shift__container'>
                    <h3 className='mb-5' style={{ fontWeight: 'bold', color: '#00275c' }}>Ticket List</h3>
...
export default Tenant_ticket_list